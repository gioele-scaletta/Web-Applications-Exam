...
const CloseAnswerForm = function (props) {
    ...
    if (props.response) {
        return ( //READONLY VERSION FOR ADMIN CHECKING RESPONSES OR CREATING SURVEY
            <Form>
                <Form.Group controlId="exampleForm.ControlTextarea1">
                    <Row>
                        <Form.Label className="m-0" style={{ fontSize: '115%', fontWeight: '600' }}>{Question[0]}</Form.Label></Row>
        ...
        return (
            <Form>
                <Form.Group controlId="exampleForm.ControlTextarea1">
                    <Row> <Form.Label className="m-0" style={{ fontSize: '115%', fontWeight: '600' }}>{Question[0]}</Form.Label></Row>

                    {(props.error !== [] && props.tried && props.error.filter((a) => a === (props.id)).length > 0) ?
                        <Form.Text style={{ textShadow: '0px 0px 10px #EEFBFB' }}> {" (For this question you can select between " + props.optional + " and " + props.single + " checkboxes)"}  </Form.Text> :
                        <Form.Text> {" (For this question you can select between " + props.optional + " and " + props.single + " checkboxes)"}  </Form.Text>}

                    <br></br>
                    {Question.map((q, index) => {
                        Response[index - 1] = 0; if (index !== 0) return (
                    ...
                    {(props.error !== [] && props.tried && props.error.filter((a) => a === (props.id)).length > 0) ? <Alert variant={'danger'}>
                        Your answer does not respect the constraints of selectable checkboxes, please correct your answer</Alert > : <><br></br><hr></hr><br></br></>}
